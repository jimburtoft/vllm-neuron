...
"""
Medusa speculative-decoding HEADS + head LOADER for native-backend Voxtral.
...
* **Head architecture (Medusa paper / NxDI-verbatim):**
  ...
  the common case; L>1 stacks L ResBlocks). d_model=3072, vocab=131072.
* **Medusa-1 weight tie (default):** the output projection of every head is the
  model's EXISTING sharded LM head (the Voxtral Llama ``lm_head``,
...
class MedusaHeads(nn.Module):
    """N Medusa-1 heads reading the Voxtral Llama decoder's LAST hidden state.
...
def load_medusa_heads(
    ...
    Args:
        ...
        hidden_size: d_model (3072 for Voxtral-Mini-3B).
